...
public:
    ...
    String(const String &address) {
        this->data = nullptr;
        if(address.data) {
            this->data = new char[strlen(address.data) + 1];
            strcpy(this->data, address.data);
        }
    }
    ...
    void append(const char *text) {
        size_t Length = getLength() + strlen(text);
        char* appendText = new char[Length + 1];
        strcpy(appendText, this->data);
        strcat(appendText, text);
        delete[] this->data;
        this->data = appendText;
    }
